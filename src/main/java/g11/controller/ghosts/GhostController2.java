...
public class GhostController2 {
    ...
    public int getTicksToEndFrightened() {
        return ticksToEndFrightened;
    }

    public void setTicksToEndFrightened(int ticksToEndFrightened) {
        this.ticksToEndFrightened = ticksToEndFrightened;
    }
     ...
     * @param ghostStateENUM parametro recebido de update de Game() para alertar estados Eaten e Frightened
     */
    public void determineState(long elapsedtime, GhostStateENUM ghostStateENUM) {
        // TODO Ainda fica preso na Ghost House

        // Caso fiquem presos dentro da casa em Chase ou Scatter
        if (ghost.getState() != GhostStateENUM.EATEN && ghost.getState() != GhostStateENUM.ENTERINGHOUSE && isInsideHouse(ghost)) {
            ghost.setState(GhostStateENUM.CHASE);
            ghost.setTarget(new Position(13, 14));
            setExitingHouse(true);
        }
        // Para não sairem e voltarem a entrar
        if (ghost.getState() != GhostStateENUM.EATEN &&
                ghost.getState() != GhostStateENUM.ENTERINGHOUSE &&
                ghost.getState() != GhostStateENUM.FRIGHTENED &&
            ...
            if (ghostStateENUM == GhostStateENUM.EATEN) {
            ...
            // Entra em FRIGHTENED
            if (ghostStateENUM == GhostStateENUM.FRIGHTENED) {
                // só põe frightened se não estiver EATEN, ENTERINGHOUSE, ou exiting
                if (ghost.getState() != GhostStateENUM.EATEN && ghost.getState() != GhostStateENUM.ENTERINGHOUSE && !isExitingHouse()) {
                    ghost.setState(GhostStateENUM.FRIGHTENED);
            ...
            if (getTicksToEndFrightened() == 0 && ghost.getState() != GhostStateENUM.EATEN && ghost.getState() != GhostStateENUM.ENTERINGHOUSE && !isExitingHouse())
                ghost.setState(setStatetime(elapsedtime));
            ...
            if (ghost.getState() != GhostStateENUM.EATEN &&
                    ghost.getState() != GhostStateENUM.ENTERINGHOUSE &&
                    ghost.getState() != GhostStateENUM.FRIGHTENED &&
                    !(isExitingHouse() && ghost.getState() == GhostStateENUM.CHASE))
                ghost.setState(setStatetime(elapsedtime));
        }
    }
        ...
        switch (ghost.getState()) {
            ...
            case EATEN:
                if (ghost.getPosition().equals(new Position(13, 14))) {
                    ghost.setState(GhostStateENUM.ENTERINGHOUSE);
                } else {
                    ghost.setTarget(new Position(13, 14));  // FIXME depende do mapa -> v2 (24, 14) ; v1 (13, 14)
                    calculateAndStep(gameData, step);
                }
    ...
    public void calculateAndStep(GameData gameData, int step) {
        ...
        if (step % 4 == 0) {
            if ((ghost.getState() == GhostStateENUM.CHASE || ghost.getState() == GhostStateENUM.SCATTER)) {
                {
            ...
            if (ghost.getState() == GhostStateENUM.ENTERINGHOUSE) {
                ghost.setOrientation(DOWN);
                ghost.moveDirection();
            }
        } else if (step % 5 == 0 && ghost.getState() == GhostStateENUM.FRIGHTENED) {
            if (changeOrientation) {
                ghost.setOrientation(ghost.getOrientation().getOpposite());
                changeOrientation = false;
            } else {
            ...
            ghost.moveDirection();
        } else if (step % 3 == 0 && ghost.getState() == GhostStateENUM.EATEN) {
            availableOris = getAvailableOrientations(gameData);
            if (availableOris.size() > 0) {
                ghost.setOrientation(chooseOrientation(availableOris));
            }
